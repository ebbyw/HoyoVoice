...
- **`Asha` reads as "Ahshaa" (`ˈɑʃɑ`), not "uh-SHA" (`əʃˈæ`).** Unhyphenated on purpose: `aa-shah` phonemizes to `ˈɑˌɑʃˌɑ`, an extra syllable, and `Ah-shah` splits the engines (misaki `ˌɑʃˈɑ`, espeak `ˈɑːʃˈɑː`). Run `python tools/pronounce_names.py --write` to pick it up — `voices.json` is yours and a pull never updates it.

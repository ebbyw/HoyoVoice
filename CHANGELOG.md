...
- **Windows support**: new `hv_platform/` backend layer — DirectShow video capture, in-process WASAPI audio capture (replaces sox's role), `tools/ocrd_win.py` OCR daemon (RapidOCR or Windows.Media.Ocr, same JSON protocol and Vision-style coordinates), Kokoro TTS via kokoro-onnx on CPU (same voice IDs — `voices.json` carries over), sounddevice playback, `setup.ps1`, and a cross-platform `hoyovoice.py` launcher. See `plans/WINDOWS-TESTING.md` for the first-run checklist.
- **Session replay harness (`tools/replay.py`).** Any dashboard recording now replays through the REAL pipeline — actual OCR daemon, classification, stabilization, dedupe, VAD gate, and yield, with only capture/TTS/playback simulated — in a throwaway state dir that can't touch real casting or caches. Every reported bug becomes a reproducible test case; most of the fixes below were diagnosed and verified with it. (`HOYOVOICE_STATE_DIR`, `HOYOVOICE_BACKEND=replay`, `HOYOVOICE_PORT` are the supporting hooks.)
...
- **The last message of a chat conversation was never read.** The panel's Scroll/Back hints sit inside the message band (center y≈0.15 against its 0.14 floor), and system notices ("Conversation Over", "… started sharing location") print in the message column. Both were parsed as message text: they attached to the final message and dragged its position below the clip-edge threshold, which exists to hold back a message still scrolling into view — so the real last message was discarded every time. Panel chrome is now excluded from the body, and "Conversation Over" marks the conversation complete so its final message is never held back. A genuinely still-scrolling message is still deferred.
- **OCR jitter on Windows.** Measured on 29 consecutive frames of one on-screen line: the engine produced 8 distinct raw reads and 4 distinct speaker reads. Almost all of that variance is punctuation/spacing micro-noise that normalizes away — but it decides *which* text gets spoken, and the reads differ in quality. Two fixes, both engine-independent: (1) the best read wins — every read of a line is scored on how many tokens are real words, and the highest-scoring variant is spoken rather than whichever frame tripped the threshold (in the measured case the correct "mercy is" was a 2-of-29 minority that majority voting would have lost); (2) run-on repair splits fused word pairs the Chinese-trained recognizer emits ("mercyis" → "mercy is") using word frequencies, with capitalized tokens protected so game proper nouns ("Wishpower", "Cindearth") are never split. Alternatives were tested and rejected: Tesseract reads the game art rather than the subtitles, and the built-in Windows engine was already worse on this font.

...
- **Snapping to the game's text** — `settings.textmap` points at a file of the game's own dialogue strings, and a settled line is matched back to the one it came from before anything else sees it. This repairs the everyday damage at the source: `Ves.` becomes `Yes.`, `Choosel"mission budget"` becomes `Choose "mission budget"`, a dropped full stop comes back (sentence streaming reads punctuation), and the repaired line then *matches the next read of itself*, so the jitter that makes a line read twice stops before dedupe ever sees it. A match has to score 0.82 and beat the runner-up by 0.05, or the read is kept exactly as it was — measured on 164 real misreads from recorded sessions, 113 were repaired to the right line, 51 refused, and none snapped to a wrong one. Refusals are on purpose: a garbled line read aloud is a worse-sounding failure than a confident wrong sentence, but a confident wrong sentence is a worse *failure*. Plain text (one line per entry), a JSON array, or a JSON object of id → line; ~11 ms per lookup against a 100k-line map, paid once per spoken line. **No such file ships with HoyoVoice** — the games' text is HoYoverse's, and extracting it from your own installation is up to you; leave the setting empty and nothing changes.
...
```jsonc
{
  ...
  "settings": {
    ...
    "textmap": "",                          // path to the game's own dialogue
                                            // strings; "" = off (see below)

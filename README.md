...
- Windows: winget (ships with Windows) — `setup.ps1` installs ffmpeg and Python itself. Any GPU is strongly recommended: `setup.ps1` installs DirectML, which runs the accurate OCR engine at roughly 0.1–0.3s/frame instead of ~4s on CPU (without it the app falls back to the built-in Windows OCR, which misreads small game fonts). `setup.ps1` also fetches an English recognition model (~8 MB) into `models\`.
...
![The dashboard mid-session: live analytics, per-character casting with the default voice slots pinned on top, the capture preview reading Star Rail, recordings, and the decision log](docs/dashboard.png)
...
- **Anchors and ROI cropping** — a small template of each game's own chrome (Star Rail's ✕-Continue glyph, Genshin's auto-play toggle), matched by pixel correlation before OCR runs. The templates aren't shipped: the app cuts them from *your* capture the first time it trusts a dialogue screen, into `captures/anchors/`. When the chrome matches, OCR reads only that screen kind's region — measured ~42% off Windows detector time — and when it doesn't, the whole frame is read exactly as before; anchors gate cost, never speech. `settings.anchors` and `settings.anchor_roi` (both default on) control the two halves, and `roi_crops` on the dashboard shows the crop volume.
...
- **Named gate priors** — the prior above is built out of the VAD's own verdicts, which puts it out of reach of a character the VAD cannot hear. Paimon and Sparxie are processed high-register voices that a speech model trained on human speech scores at or near zero, so their voiced lines get talked over, each talk-over is filed as another unvoiced observation, and the record that would have softened the gate never arrives. Both are named in the app as **model-deaf**: the center-energy detector, which measures a real center-channel burst rather than speechiness, is allowed to call their lines voiced on its own, and at a lower bar — 6dB of center-over-side rather than 8. That number comes from one line read four times over the game's own delivery of it, at 7.1, 6.4, 6.7 and 9.8dB; only the last crossed the usual cut, so three of the four were talked over. The lower bar applies only while something in the scene has been heard to be voiced in the last ten minutes, whoever was speaking — in a quest with no voice acting at all it would silence lines nothing else is reading. Their VAD thresholds are untouched, for the same reason: Paimon spends hundreds of lines unvoiced. `settings.gate_prior` extends the list and adds two stronger declarations, `"voiced"` (always voiced: soft gate from the first line) and `"unvoiced"` (never voiced: hold the firm gate so a blip can't cut their lines short).
...
- **Snapping to the game's text** — `settings.textmap` names a file of the game's own dialogue strings, per game (`{"genshin": "…/TextMapEN.json", "hsr": "…/TextMapEN.json"}`; a bare string is taken as the current game's). A settled line is matched back to the line it came from before anything else sees it, which repairs the everyday damage at the source: `Ves.` becomes `Yes.`, `Choosel"harbor repairs"` becomes `Choose "harbor repairs"`, a dropped full stop comes back (sentence streaming reads punctuation), and the repaired line then *matches the next read of itself*, so the jitter that makes a line read twice stops before dedupe ever sees it. A match has to score 0.90, beat the runner-up by 0.05, and keep every real word the read contained, or the read is kept exactly as it was. Refusals are the point: a garbled line read aloud sounds worse, but a confident wrong sentence *is* worse — and on a real 315k-line dump that was not a hypothetical. The first thresholds were calibrated against a 377-line map, where they repaired 113 of 164 real misreads and got none wrong; against a map 800 times larger there is a plausible near-neighbour for almost every short line, and 23 of the 30 word-changing snaps in the logs were the wrong line ("We're a legitimate organization." → "We are a flat organization"). Raising the score and requiring the read's own words to survive the match — OCR damage lands on non-words, so a match that drops `legitimate` or a name is a different sentence rather than a repair — leaves 1 wrong line in those 30 instead of 23.

  Sentence streaming means the text handed to the matcher is often only the *first sentence* of the line, which cannot match its own entry on length: 31 characters against that line's 59. So each entry's opening sentence is indexed as a form of the entry in its own right, and a streamed partial repairs to its own first sentence and stops there, never running ahead of the typewriter. Measured on both current dumps, that is 327k entries becoming 448k on Star Rail 4.5.0 and 398k becoming 549k on Genshin 7.0.0 — about 1.38× either way — for +0.05 and +0.07 GB resident and a few seconds more to index, because first sentences are short and contribute few postings. Widening the length bound instead does not work and was measured: scored whole-string, a half-typed read rates 0.53–0.65 against the line it came from and *loses* to the wrong neighbour of its own length at 0.76–0.82, so no threshold admits the right line without preferring the wrong one.

  Set `settings.player_name` to your character's in-game name. Thousands of entries carry a `{NICKNAME}` placeholder the game substitutes at runtime, and without it every one of those lines is unmatchable. The rest of a dump's markup is unwrapped the way the game draws it — the `#` sentinel, `{F#…}{M#…}` gender pairs (both indexed, since the game picks one), `{RUBY#…}` glosses (drawn above the word, not in the line), `<color>`/`<unbreak>`/`<i>` spans, escaped newlines — and an entry still holding a runtime placeholder is dropped rather than indexed subtly wrong. A map loads on first use of its game, so the other game's map is never built for a session that doesn't read it. Measured on the current dumps: Star Rail 4.5.0 is 466k raw entries → 448k usable lines, 10.4 s to index and 0.76 GB resident; Genshin 7.0.0 is 586k → 549k, 15.2 s and 0.71 GB. A lookup is ~7 ms, paid once per spoken line. **Take the dump from the repository's default branch** — one of these repos keeps a long-stale `master` alongside its real `main`, and the stale one looks perfectly valid: it parses, it is less than half the size, and it scores 1 line in 200 instead of 132.
...
- **Pronunciations** — `settings.pronunciations` substitutes spoken forms at synthesis only ("Wishpower" → "Wish power"); logs, dedupe and casting keep the real spelling. Ships with ~80 character names Kokoro reads wrong (`python tools/pronounce_names.py --check` reports the exact count), because it applies English spelling rules to pinyin and romaji: x becomes /z/ ("Xiao" → "ZY-ah-oh"), q becomes /k/ ("Qiqi" → "KIH-kee"), zh becomes /ʒ/, and a final -e disappears ("Shenhe" → "shenh"). Lore terms live in the same map but a separate table (`TERMS`), since no roster lists them and the coverage report can't check them — an invented word is exactly what English spelling rules mangle, so they ship for the same reason, and `--custom-words` feeds them to OCR too. `python tools/pronounce_names.py` prints what the synthesizer says for every entry with and without its respelling, checked against the same phonemizer Kokoro uses; `--check` reports what your own `voices.json` would actually say and what it's missing; `--write` merges the table in — and the rosters' documented genders into `settings.genders`, which auto-casting trusts over the name-shape guess — and `--custom-words` also feeds both games' full rosters to the OCR vocabulary. Matching is case-insensitive so OCR case jitter can't miss a name — for a name that is *also* an ordinary word ("Gaming", and any entry you add for Jade, Sunday, Hook, Blade, Archer, Robin), list it in `settings.pronunciations_exact` and only the capitalised spelling is respelled.

  **Find the next one before you hear it:** every entry in that table arrived after somebody heard the app say a name wrong, mid-scene. If you have a TextMap dump (same file `settings.textmap` wants), `python tools/textmap_words.py` reads both games' whole vocabulary out of it and reports what the synthesizer will get wrong, commonest first — words the phonemizer's lexicon misses and has to guess at from English spelling rules, with the rule that bit named (a pinyin x read as /z/, a dropped final -e, the flat /æ/ of "fat", a doubled r, a flapped t), and words the lexicon *has* whose espeak reading disagrees, which are right on macOS and wrong on Windows. `--names-only` keeps the ones seen capitalised mid-sentence, `--class split` the Windows-only ones. What comes out is candidates with both engines' readings side by side, not respellings: choosing one means reading `pronounce_names.py`'s header and checking by ear.
...
- **Choice prompts** — one or two options are read aloud (a lone option is the game putting words in the player character's mouth; a pair reads as the player weighing their answer — the two texts joined by an ellipsis, the beat between alternatives), always after the line they sit above and only into a gap in the talking. They're cast under the player character's name for the current game — `Traveler` in Genshin, `Trailblazer` in Star Rail — so the prompt appears in Casting like anyone else and you can give it whatever voice you want; `settings.choice_speaker` overrides the name. Three or more options are a menu, so they are logged and left unspoken. Either way the prompt appears in the dashboard log, including when it went unread.
...
```jsonc
{
  ...
  "settings": {
    ...
    "ocr_engine": "auto",                   // Windows only: auto | rapid |
    ...
    "genders": {"Paimon": "female"},        // auto-cast pool hints; seeded
                                            // by pronounce_names --write
                                            // from the games' rosters
    ...
    "anchors": true,                        // match game-chrome templates,
                                            // self-calibrated into
                                            // captures/anchors/ from your
                                            // own feed on first dialogue
...
Where those files come from: any Kokoro-82M voice repo (`hexgrad/Kokoro-82M` is the upstream one). The packaged model's own ~54 voices are all in the menu already (minus the broken `af_nicole`) — the English ones (`af_`/`am_` American, `bf_`/`bm_` British) plus Spanish (`ef_`/`em_`), French (`ff_`), Hindi (`hf_`/`hm_`), Italian (`if_`/`im_`), Japanese (`jf_`/`jm_`), Portuguese (`pf_`/`pm_`) and Mandarin (`zf_`/`zm_`). The non-English ones speak English text through the American English phonemizer, so what you get is a different-sounding *speaker*, not a language switch — audition before casting, since their English varies from a pleasant accent to barely intelligible.
...
| `tools/ocr_bench.py` | Measure the OCR: frame corpora (`extract`/`capture`) scored for stability, accuracy and stabilization policy |
...
| `tools/textmap_words.py` | Scan a TextMap dump for words the synthesizer will mispronounce, ranked by how often the games say them |
...
- **Windows: lines are slow to appear or misread.** Check the startup log for the OCR engine: `engine: rapid (directml, …)` is the good path — the startup probe reads ~115–194 ms, and real dialogue frames average ~320 ms with the default ROI cropping. Judge health by `ocr_avg_ms` in the dashboard, not the probe; over ~600 ms average will lag the 6 fps loop. If it says `windows`, DirectML didn't install — rerun `setup.ps1`, or `.venv\Scripts\pip install onnxruntime-directml`. The built-in Windows engine is only a fallback and misreads small game fonts. Force a choice with the `HOYOVOICE_OCR_ENGINE` environment variable (`auto`, `rapid`, `windows`).
